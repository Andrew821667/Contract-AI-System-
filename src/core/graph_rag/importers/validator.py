...
# Маркеры заглушки «некоммерческой версии» К+ (документ доступен только «по
# расписанию»). Markdown-конвертер дробит фразы ссылками («доступен [по
# расписанию](...)»), поэтому маркеры — короткие устойчивые подстроки.
BLOCKERS = (
    'по расписанию',
