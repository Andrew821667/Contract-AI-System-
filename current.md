...
**Статус:** предустановленное согласие на стартовой странице убрано и развернуто в production.
**Ветка:** fix/demo-consent-default

---

## ТЕКУЩАЯ ЗАДАЧА — СОГЛАСИЕ ТОЛЬКО ЯВНЫМ ДЕЙСТВИЕМ

**Цель:** стартовая форма входа не должна отмечать согласие за пользователя, даже если прежнее принятие сохранено в браузере.

- [x] Убрать восстановление галочки из `localStorage` на главной странице.
- [x] Сделать checkbox нативно обязательным.
- [x] Добавить регрессионный e2e-тест с заранее заполненным `localStorage`.
- [x] Проверить type-check, production build и браузерный сценарий.
- [x] Адресно развернуть frontend и проверить production.

### 2026-09-22 — согласие на главной начинается с пустой галочки
- Сохранённое ранее согласие остаётся в профиле и используется там, где это необходимо, но больше не подставляется в форму нового входа.
- Каждая новая загрузка стартовой страницы требует самостоятельного действия пользователя.
- TypeScript-проверка, production-сборка и Playwright-сценарий с заранее заполненным `localStorage` прошли успешно.
- Frontend-контейнер `contract-ai-frontend:0d8dff1` развернут без перезапуска backend; в production подтверждены HTTP 200, пустая галочка, атрибут `required` и отсутствие ошибок страницы.

---

## ПРЕДЫДУЩАЯ ЗАДАЧА — ДЕМО НА DEEPSEEK V4.1 FLASH

**Цель:** перед выдачей демо-доступа по заявке обновить ключ DeepSeek, сделать V4.1 Flash моделью по умолчанию, честно сказать клиенту, что демо идёт на облегчённой модели, и прогнать весь сценарий демо на production.

- [x] Ключ DeepSeek заменён в production `.env` (скрытый ввод), права файла ужесточены до 600, backend перезапущен; список моделей по ключу: `deepseek-flash`, `deepseek-v4-pro`.
- [x] Реестр моделей: Flash = `deepseek-flash` (V4.1, с 10.09.2026); `deepseek-v4-flash` и `-vision-exp` — псевдонимы; публичные названия в одном месте; цены Flash по прайсу сентября ($0.30 / $1.20 за 1M).
- [x] Миграция `026_deepseek_v41_flash`: сохранённые маршруты и политики переименованы на `deepseek-flash`.
- [x] Примечание клиенту «демо на облегчённой модели, по подписке — самые мощные»: страница демо (заявка и активация), приветствие демо-пользователя, счётчик лимитов, тарифы.
- [x] Скрипт `scripts/deploy_prod_macmini.sh`: код → миграции → backend → образ фронта → единственный контейнер на 3103 → проверка снаружи.
- [x] Опубликовать PR, дождаться CI, выкатить на production скриптом (#76 → #79).
- [x] Сквозной прогон демо в браузере по тестовой ссылке: активация → загрузка договора → анализ → результат → лимиты → примечание. Найдено и исправлено по ходу: выбор стороны анализа (409 терял `code/parties`, #77), лишняя модалка смены пароля у демо (#77), пустые правки/аннотации из-за обрыва ответа по `max_tokens` (#78), AI-панель без текста договора (#79 — не тот слой, #80 — сам маршрут).
- [x] Выдать доступ по реальной заявке: заявка от 16.09 одобрена серверным скриптом с логикой approve-эндпоинта; лимиты 3 договора / 15 AI-запросов / 72 ч; ссылка привязана к email заявителя и передана владельцу.

### 2026-09-16 — найдено на production до изменений
- Публичный домен обслуживал rollback-контейнер `d03f963` (11.08): после перезагрузки хоста 14.09 он занял порт 3103, а актуальная сборка `bab7af0` поднялась без порта. Backend был актуальным.
- В production `.env` стояло `DEEPSEEK_MODEL=deepseek-v3` (нормализовалось на лету), файл был читаем всем пользователям хоста.
- Демо и до изменений шло только через Flash: анализ жёстко использует `llm_quick_model`, глубокий анализ (Pro) демо-роли недоступен, задачи AI-панели не доходят до порога expert.
- Маршрут AI-панели `POST /api/v2/ai/sessions/{id}/messages` не использует `AICollaboratorService`: он вызывает LLM напрямую и собирал контекст из `meta_info.full_text`, которого никто не пишет. Теперь контекст документа для него строит тот же `AIContextBuilderService` и общий блок `render_document_context` (#80).
- После #78 на production: 20 рисков, 17 рекомендаций, 22 правки, 22 аннотации по тестовому договору (до этого правки и аннотации были 0).

---

## ПРЕДЫДУЩАЯ ЗАДАЧА — DEEPSEEK FLASH ПО УМОЛЧАНИЮ

**Цель:** вернуть экономичную двухуровневую схему: Flash обслуживает обычную работу без thinking, Pro включается только для глубоких и экспертных задач.

- [x] Добавить единый реестр актуальных моделей и цен.
- [x] Явно отключить thinking для Flash и включить high-effort thinking для Pro.
- [x] Перевести gateway, прямые клиенты, RAG и ассистента на единую политику.
- [x] Оставить Pro только на `expert`, высокой сложности и `analyze_deep`.
- [x] Нормализовать старые значения из окружения, Redis и БД без ручной миграции.
- [x] Обновить админку, frontend-селекторы и тестовые сценарии.
- [x] Добавить изолированные тесты без live LLM API-вызовов: 11 passed.
- [x] Опубликовать и проверить production без тестового расходования LLM-токенов.

### 2026-08-25 — повторный полный аудит
- [x] Проверены tracked-код, скрипты, тесты, документация, compose/CI, Redis override и запущенные процессы.
- [x] Удалены два опасных live-теста; RAG eval и остальные live-тесты требуют явного подтверждения платного запуска.
- [x] Прямые OpenAI-compatible клиенты централизованно выбирают DeepSeek endpoint для Flash/Pro.
- [x] Неявный gateway всегда начинает с DeepSeek Flash, даже если в старом окружении остался default OpenAI.
- [x] Порог автоматического перехода на Pro выровнен до `0.8`; сам факт скана больше не включает Pro.
...
- Изолированные тесты в production Python 3.11: 11 passed; переменные LLM-ключей удалены из окружения тестового процесса.
- Backend и frontend пересобраны/перезапущены отдельно; локальный и публичный health, а также главная страница возвращают HTTP 200.
- Live-запросы к DeepSeek, OpenAI, Anthropic и другим LLM API для проверки не выполнялись.

### Обязательная LLM-политика
- Стандартная модель: `deepseek-flash` (DeepSeek V4.1 Flash; прежнее имя `deepseek-v4-flash` — псевдоним), всегда `thinking: disabled`.
- Думающая модель: `deepseek-v4-pro`, только explicit expert/deep routes, `thinking: enabled`, `reasoning_effort: high`.
- Старые `deepseek-chat`, `deepseek-v3`, `deepseek-v3.2` в стандартном поле становятся Flash; старые значения глубокого поля становятся Pro.
- Unit-тесты не должны обращаться к live LLM API. Все ключи LLM при таких проверках удаляются из окружения процесса.

---

## ПРЕДЫДУЩАЯ ЗАДАЧА — ДЕМО-ДОСТУП ТОЛЬКО ПО ЗАЯВКЕ

**Цель:** закрыть публичную регистрацию и выдавать ограниченный персональный демо-доступ только после ручного рассмотрения заявки.

**Границы:** только standalone Contract AI; LawTable и другие сервисы AI Verdict не затрагивать.

- [x] Закрыть legacy endpoint публичной регистрации и перенаправить `/register` на `/demo`.
- [x] Добавить заявку с контактом, задачей, согласием, honeypot и дедупликацией.
- [x] Добавить Telegram-уведомление без потери заявки при сбое уведомления.
- [x] Добавить список, одобрение и отклонение заявок в React-админке.
- [x] Привязать одноразовую ссылку к email и production-домену.
- [x] Применить лимиты приглашения ко всему демо, без ежемесячного сброса.
- [x] Сохранить версионное согласие при входе, активации демо и загрузке документов.
- [x] Обновить публичные тексты, SEO, условия и политику конфиденциальности.
- [x] Удалить старую отдельную админку, её контейнер, маршруты, зависимости и документацию.
- [x] Проверить API-тесты, type-check, production build и desktop/mobile.
- [x] Сверить параллельные изменения пользователя, опубликовать PR и проверить production.

### 2026-07-16 — персональное демо по заявке
- Добавлена миграция `024_demo_access_requests` и модель заявок.
- Публичная заявка не создаёт пользователя; аккаунт появляется только при активации одобренной ссылки.
- Демо-ссылка ограничена email, сроком, количеством договоров и общим количеством AI-запросов.
- Отдельная Python-админка удалена вместе с compose-сервисом, proxy-маршрутами, launchd, зависимостью и устаревшими руководствами; `/admin` обслуживается Next.js.
- Параллельная production-правка согласий объединена с закрытой регистрацией: согласие записывается версионно, а загрузка без него блокируется.
- Проверки: 98 API/auth/quota/upload/consent тестов, auth/admin script, TypeScript, production build, compose config, Alembic head, desktop/mobile screenshots.
- PR #52 опубликован в production; backend, публичный и нативный frontend проверены, прежний маршрут возвращает 404, старые frontend-контейнеры и образы удалены.

---

## ПРЕДЫДУЩАЯ ЗАДАЧА — HERO-ФОН СТАРТОВОЙ СТРАНИЦЫ

### 2026-07-16 — тематический hero Contract AI
- Добавлен локальный оптимизированный фон с договором и визуализацией AI-проверки.
- Фон подключён только к hero главной страницы; остальные auth-страницы не изменены.
- Desktop и mobile проверены в браузере: переполнения и ошибок консоли нет.

---

## ПРЕДЫДУЩАЯ ЗАДАЧА — КОНТРАСТ ТЕМ CONTRACT AI
...
- [x] Опубликовать PR, дождаться CI и проверить production.
- [x] Подпись и пиктограмма переключателя показывают текущую тему.
- [x] Действие кнопки описывает следующую тему через `aria-label` и `title`.
- [x] Опубликовать исправление подписей и проверить production.
- [x] Удалить устаревший пункт отдельной админки из sidebar.
- [x] Переименовать `/admin` из «Управление» в «Админ-панель».
- [ ] Опубликовать исправление ссылки и проверить production.
...
### 2026-07-15 — корректные названия тем
- Название и пиктограмма теперь отражают текущую тему, а не следующую в цикле.
- Логотип в steel использует светлый вариант и сохраняет контраст.

### 2026-07-15 — полноценная админ-панель
- Sidebar ведёт на единственную React-админку `/admin`.
- Дублирующий legacy-пункт удалён; права `canManageUsers` сохранены.
...
7. **Каскадная LLM** — orchestration/agent (DeepSeek Flash) → expert/deep (DeepSeek Pro) → tool (deterministic)
...
### Этап 5 — Извлечённые клаузулы как внутренние данные ✅
