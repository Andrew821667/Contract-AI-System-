...
**Статус:** демо переведено на DeepSeek V4.1 Flash с примечанием клиенту; деплой и сквозной прогон демо перед выдачей доступа по заявке.
**Ветка:** main
...
- [ ] Опубликовать PR, дождаться CI, выкатить на production скриптом.
- [ ] Сквозной прогон демо в браузере по тестовой ссылке: активация → загрузка договора → анализ → результат → лимиты → примечание.
- [ ] Выдать доступ по реальной заявке.
