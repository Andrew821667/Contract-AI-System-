...
**Статус:** Тематический hero-фон стартовой страницы Contract AI.
**Ветка:** test/contract-hero-background

---

## ТЕКУЩАЯ ЗАДАЧА — HERO-ФОН СТАРТОВОЙ СТРАНИЦЫ

**Цель:** добавить на стартовую страницу Contract AI собственный локальный фон в общей стилистике AI Verdict, связанный с анализом договоров.

**Границы:** менять только hero стартовой страницы и локальный графический asset; не затрагивать остальные auth-страницы, аутентификацию, договорную логику, SEO-разметку и API.

- [x] Создать тематический фон с договором и ненавязчивыми AI-элементами.
- [x] Подключить локальный asset только к hero стартовой страницы.
- [x] Настроить наложение и кадрирование для desktop/mobile.
- [x] Проверить type-check, lint, тесты, production build и ключевые viewport.
...
- [x] Удалить legacy-пункт `/streamlit-admin/` из sidebar.
...
- Sidebar ведёт на React‑админку `/admin` вместо устаревшего Streamlit-маршрута.
