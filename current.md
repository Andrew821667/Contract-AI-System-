...
**Статус:** Коррекция цветовой системы Contract AI по палитре основного сайта AI Verdict.
**Ветка:** test/contract-ai-light-palette

---

## ТЕКУЩАЯ ЗАДАЧА — СВЕТЛАЯ ПАЛИТРА AI VERDICT

**Цель:** синхронизировать цветовую систему публичного входа и авторизованного интерфейса Contract AI со светлой палитрой основного сайта AI Verdict.

**Границы:** не менять аутентификацию, договорную логику, SEO-разметку, тарифные расчёты и API.

- [x] Сопоставлены фактические палитры двух production-сайтов.
- [x] Светлая тёплая база, белые панели, графитовый текст и янтарные CTA применены к публичным страницам.
- [x] Светлая тема сделана темой по умолчанию; sidebar и top bar кабинета больше не принудительно тёмные.
- [x] Проверить type-check, сборку, тесты и ключевые экраны локально.
- [ ] Опубликовать PR, дождаться CI и проверить production.

### 2026-07-15 — коррекция цветовой гаммы
- Убрана принудительная тёмная оболочка, которая визуально отделяла Contract AI от AI Verdict.
- Cyan сохранён только как вторичный фирменный акцент; основные действия остаются янтарными.
