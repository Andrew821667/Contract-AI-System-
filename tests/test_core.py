...
class TestLLMRoutingPolicy:
    ...
    def test_apply_policy_sensitivity_override(self):
        ...
        svc = LLMRoutingPolicyService(db=None)  # type: ignore[arg-type]
        model, reason = svc.apply_policy("deepseek-flash", policy, sensitivity="high")
...
class TestFallbackHandler:
    ...
    def _fresh_handler(self):
        ...
        for model in ["deepseek-flash", "deepseek-v4-pro", "gpt-4o", "gpt-4o-mini"]:
            handler.clear_failures(model)
        return handler

    def test_circuit_breaker(self):
        handler = self._fresh_handler()
        assert handler.is_healthy("deepseek-flash") is True

        # Record failures
        for _ in range(3):
            handler.record_failure("deepseek-flash")

        assert handler.is_healthy("deepseek-flash") is False
    ...
    def test_get_healthy_models(self):
        ...
        healthy = handler.get_healthy_models(["deepseek-flash", "gpt-4o", "gpt-4o-mini"])
        assert "deepseek-flash" in healthy
    ...
    def test_clear_failures(self):
        ...
        handler.clear_failures("deepseek-flash")
        assert handler.is_healthy("deepseek-flash") is True
